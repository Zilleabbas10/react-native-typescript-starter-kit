    ...
    return (
      <View style={[{ position: "relative", width: width, height: height, zIndex }, style]}>
        <Svg
          ...
          width={(width + lineWidth * 2)} style={{ position: "absolute", top: y - lineWidth, left: x - lineWidth }}>
          <Defs>
            <RadialGradient
              id="grad"
              cx={width / 2}
          ...
          <Ellipse cx={width / 2} cy={height} rx={shadowRadius} ry={Platform.OS === 'android' ? '50' : '55'} fill="url(#grad)" />
        </Svg>
        {children}
      </View >
    )
